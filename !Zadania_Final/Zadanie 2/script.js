...
let updateTodoList = function () {
    ...
    for (let i = 0; i < todoList.length; i++) {
        for (let j = 0; j < todoList.length - 1; j++) {
            if (todoList[j].dueDate > todoList[j + 1].dueDate) {
                let addVar = todoList[j];
    ...
    for (let todo in todoList) {
        ...
        if (
            (((filterInput.value.toString() === "") ||
                (todoList[todo].title.toLowerCase().includes(filterInput.value.toLowerCase())) ||
                (todoList[todo].description.toLowerCase().includes(filterInput.value.toLowerCase()))) && flag)
        ) {
...
let checkDate = function (startDate, endDate, dueDate) {
    ...
    if (startDate.toString() === "" && endDate.toString() === "") {
        returnValue = true
    } else if (startDate.toString() === "" && endDate.toString() !== "" && dueDate <= endDate) {
        returnValue = true
    } else if (startDate.toString() !== "" && endDate.toString() === "" && dueDate >= startDate) {
        returnValue = true
...
(() => {
    ...
    // Add event listener waiting for clicking submit button
